...
class DetailGen3DPipeline(
    ...
    def prepare_latents(
        ...
        if latents is not None:
            ...
            # Convert noise_aug_level (0-1 float) to timestep (integer)
            # noise_scheduler.config.num_train_timesteps is typically 1000
            num_timesteps = getattr(self.noise_scheduler.config, 'num_train_timesteps', 1000)
            timestep = int(noise_aug_level * (num_timesteps - 1))
            timestep_tensor = torch.tensor([timestep], device=device, dtype=torch.long)
            if noise_aug_level > 0:
                latents = self.noise_scheduler.add_noise(latents, torch.randn_like(latents), timestep_tensor)
            return latents
